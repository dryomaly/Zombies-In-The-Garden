...
def draw_game_over(screen, events, score=0):
    ...
    if _clicked(events, SCREEN_W // 2 - bw - 15, 425, bw, bh):
        return "retry"
    if _clicked(events, SCREEN_W // 2 + 15, 425, bw, bh):
        return "menu"
    return None
...
def draw_pause_menu(screen, events):
    ...
    if _clicked(events, SCREEN_W // 2 - bw - 15, 380, bw, bh):
        return "resume"
    if _clicked(events, SCREEN_W // 2 + 15, 380, bw, bh):
        return "menu"
    return None
