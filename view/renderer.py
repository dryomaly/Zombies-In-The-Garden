...
_TOOLTIPS = {
    PEASHOOTER: [
        "Стрелок",
        "Урон: 10  |  Дальность: 3.5 тайла",
        "Скорость: 1.0 атк/с  |  Тип: снаряд",
    ],
    BOXER: [
        "Боксёр",
        "Урон: 25  |  Дальность: 1.2 тайла",
        "Скорость: 1.5 атк/с  |  Тип: мгновенно",
    ],
    MACHINEGUN: [
        "Пулемёт",
        "Урон: 8   |  Дальность: 4.5 тайла",
...
class Renderer:
    ...
    def _draw_grid(self, screen, model):
        ...
        for col in range(tm.cols):
            for row in range(tm.rows):
                ...
                if pos == tm.base_pos:
                    pygame.draw.rect(
                        screen, (172, 142, 98), (x, y, TILE_SIZE, TILE_SIZE)
                    )
                    ...
                    my2 = y + TILE_SIZE // 2
                    pygame.draw.line(
                        screen, ROAD_DARK, (mx2 - 6, my2), (mx2 + 6, my2), 1
                    )
                elif pos in tm.plantable and pos not in model.plants:
                    pygame.draw.rect(
                        screen, PLANTABLE_COLOR, (x, y, TILE_SIZE, TILE_SIZE)
                    )
                    pygame.draw.rect(
                        screen, GRASS_DARK, (x, y, TILE_SIZE, TILE_SIZE), 1
                    )
                else:
        ...
        pygame.draw.polygon(
            screen, (200, 75, 75), [(sx - 10, sy), (sx + 8, sy - 8), (sx + 8, sy + 8)]
        )
    ...
    def _draw_plants(self, screen, model):
        for (col, row), plant in model.plants.items():
            ...
            # Радиус атаки при наведении курсора
    ...
    def _draw_hud(self, screen, model):
        ...
        if wm.state == "countdown" and wm.wave_number < len(wm.waves):
            wtxt = (
                f"Волна {wm.wave_number+1}/{len(wm.waves)} через {wm.get_countdown()}с"
            )
        else:
        ...
        hp = self.font_med.render(
            f"База: {model.base.hp}/{model.base.max_hp}", True, (220, 100, 100)
        )
    ...
    def _draw_tooltip(self, screen, ptype, anchor_x):
        ...
        pygame.draw.rect(
            screen, (18, 44, 10), (tip_x, tip_y, tip_w, tip_h), border_radius=6
        )
        pygame.draw.rect(
            screen, (80, 150, 40), (tip_x, tip_y, tip_w, tip_h), 1, border_radius=6
        )
