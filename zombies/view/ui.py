...
def _btn(screen, text, x, y, w, h, font, color=(50,110,25), hover=(80,155,40)):
...
def draw_level_select(screen, events, scores=None):
    ...
    levels = [("Уровень 1", "Первые шаги"), ("Уровень 2", "Всё серьёзнее"), ("Уровень 3", "Последний рубеж")]
    cw, ch = 240, 165
    ...
    for i, (name, sub1) in enumerate(levels):
        ...
        d = fs.render(sub1, True, (175,220,135))
        screen.blit(d, (cx+cw//2 - d.get_width()//2, cy_card+60))
        key = f"level{i+1}"
        if key in scores:
...
def draw_settings(screen, events, settings):
    ...
    fbk = pygame.font.SysFont("Arial", 20)
    items = [("Музыка", "music"), ("Звуковые эффекты", "sounds")]
    for i, (label, key) in enumerate(items):
        ...
        _btn(screen, btn_txt, SCREEN_W//2+60, y, 155, 48, fb, color=btn_col, hover=btn_hov)
